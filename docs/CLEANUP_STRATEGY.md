...
有原生清理命令的工具只使用该命令；命令不可用或执行失败时跳过该工具，并返回错误，不会改为删除目录。仅无原生命令的工具使用目录删除：

- **原生命令**：工具自行管理缓存清理，具体范围以工具行为为准
- **目录删除**：直接删除缓存目录，下次使用时自动重建

原生命令可能清理该工具管理的其他缓存，范围不一定与扫描时列出的目录完全相同。若部分工具清理成功而其他工具失败，命令返回非零状态，已成功的操作不会回滚。预览和执行中显示的大小均为估计值。
...
# 需要检查临时、交换、备份或日志文件时，显式加入 --include-files
disk-cleaner clean --path ~/projects --include-files --dry-run
```
